...
public class BadBuyersController {

	@FXML
	private ListView<String> listBadBuyers;

	ObservableList<String> obListBadBuyers = FXCollections.observableArrayList();
	...
	public void showBadBuyers() {
									...
									if (listCustomers.get(l).getCustomerId() == listBills.get(k).getCustomer()
										...
										for (int m = 0; m < listProducts.size(); m++) {
											if (listProducts.get(m).getProductId() == listProviderProducts.get(i)
												...
												obListBadBuyers.add("Name: " + listCustomers.get(l).getCustomerName()
		...
		listBadBuyers.setItems(obListBadBuyers);
	}
